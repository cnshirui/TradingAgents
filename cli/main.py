...
def _get_preset_selections(preset_file: Path) -> dict:
    ...
    return {
        ...
        "quick_think_llm": preset.quick_llm,
        "deep_think_llm": preset.deep_llm,
...
def get_user_selections(preset_file: Path | None = None):
    ...
    A preset file answers every question up front and is not remembered as the
...
def _default_report_path(config: dict, ticker: str) -> Path:
    # This fork keeps reports beside the presets under ./reports so they can be
    # committed with the repo (upstream writes under results_dir instead).
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    return Path.cwd() / "reports" / f"{safe_ticker_component(ticker)}_{timestamp}"


def _save_report_with_default_path(final_state, config: dict, ticker: str) -> None:
    save_path = _default_report_path(config, ticker)
    try:
...
def run_analysis(checkpoint: bool | None = None, portfolio=None, preset_file: Path | None = None):
    # First get all user selections
    if preset_file is not None:
        selections = _get_preset_selections(preset_file)
    else:
    ...
    if preset_file is not None:
        _save_report_with_default_path(final_state, config, selections["ticker"])
        return

    # Prompt to save report
    save_choice = typer.prompt("Save report?", default="Y").strip().upper()
    if save_choice in ("Y", "YES", ""):
        default_path = _default_report_path(config, selections["ticker"])
...
def analyze(
    ctx: typer.Context,
    preset_file: Path | None = typer.Option(
        None,
        "--preset", "-p",
        help="Optional key=value preset file (for example MU.txt) that answers "
        "every prompt; looked up as given, then under ./stocks/.",
    ),
...
def _run_cli_analysis(checkpoint, clear_checkpoints, portfolio, preset_file):
    ...
    run_kwargs = {"checkpoint": checkpoint, "portfolio": portfolio_context}
    if preset_file is not None:
        run_kwargs["preset_file"] = preset_file
    try:
